...
object Function {
  ...
  /**
    ...
    */
  def map2[T1, T2, R](t1y: Try[T1], t2y: Try[T2])(f: (T1, T2) => R): Try[R] = for(t1 <- t1y; t2 <- t2y) yield f(t1, t2) // TO BE IMPLEMENTED 4
  ...
  /**
    ...
    * @return a value of R, wrapped in Try
    */
  def map3[T1, T2, T3, R](t1y: Try[T1], t2y: Try[T2], t3y: Try[T3])(f: (T1, T2, T3) => R): Try[R] = for(t1 <- t1y; t2 <- t2y; t3 <- t3y) yield f(t1, t2, t3)// TO BE IMPLEMENTED 5
