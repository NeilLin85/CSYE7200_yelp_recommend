...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    // 16 points.
    def getURLs(ns: Node): Seq[URL] = for (cn <- ns \\ "a"; str <- cn.attributes.value) yield new URL(u, str.mkString)// TO BE IMPLEMENTED
    ...
    // 9 points.
    for (a <- getURLContent(u); c <- MonadOps.asFuture(getLinks(a))) yield c // TO BE IMPLEMENTED
  }
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable, Seq[URL]]]] = {
    ...
    // TO BE IMPLEMENTED
    Future.sequence[Either[Throwable, Seq[URL]], Seq](MonadOps.sequence[Seq[URL]](us2 map wget))
  }
