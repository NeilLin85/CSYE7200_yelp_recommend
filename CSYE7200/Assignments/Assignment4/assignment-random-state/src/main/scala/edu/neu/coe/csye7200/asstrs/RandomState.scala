...
trait RandomState[T] {
  ...
  def flatMap[U](f: T=>RandomState[U]): RandomState[U] = f(get) // TO BE IMPLEMENTED 10 points

  /**
    * @return a stream of T values
    */
  //Hint: This a recursively method and it concatenate current element with following elements.
  def toStream: Stream[T] = Stream.cons(get, next.toStream) // TO BE IMPLEMENTED 12 points
}
...
case class JavaRandomState[T](n: Long, g: Long=>T) extends RandomState[T] {
  ...
  def next: RandomState[T] = JavaRandomState((new Random(n)).nextLong(), g) // TO BE IMPLEMENTED 7 points
  //Hint: Think of the input and output.
  def get: T = g(n) // TO BE IMPLEMENTED 5 points
  //Hint: This one need function composition.
  def map[U](f: (T) => U): RandomState[U] = JavaRandomState(n, x => f(g(x))) // TO BE IMPLEMENTED 13 points (alternatives exist)
}
...
object RandomState {
  ...
  val longToDouble: Long=>Double = {x => x.toDouble/Long.MaxValue} // TO BE IMPLEMENTED 4 points
